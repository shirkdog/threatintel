...
while (1) {
	...
	my $sensorid=$data[0];
	unless ($data[1]) {
		$dbh->do("INSERT INTO sensor (hostname,interface,detail,encoding) VALUES ('$d_read[0]','$d_read[1]',1,0)") or croak($dbh->errstr);
	...
	$dbh->do("INSERT INTO event (sid,cid,signature,timestamp) VALUES ('$sensorid','$cid','$d_read[6]','$year-$mon-$mday $hour:$min:$sec')") or croak($dbh->errstr);
	$dbh->do("INSERT INTO iphdr (sid,cid,ip_src,ip_dst,ip_proto) VALUES ('$sensorid','$cid','$d_read[11]','$d_read[12]','$d_read[15]')") or croak($dbh->errstr);
	
	my $proto;
	
	# TCP packet info
	if ($d_read[15] == 6){
		$dbh->do("INSERT INTO tcphdr (sid,cid,tcp_sport,tcp_dport,tcp_flags) VALUES ('$sensorid','$cid','$d_read[13]','$d_read[14]',0)") or croak($dbh->errstr);
		# tag hash population
		$proto="TCP";
	}
	...
	# UDP packet info
		...
		$dbh->do("INSERT INTO udphdr (sid,cid,udp_sport,udp_dport) VALUES ('$sensorid','$cid','$d_read[13]','$d_read[14]'')") or croak($dbh->errstr);
		# tag hash population
		$proto="UDP";
	}
	...
	# ICMP packet info
		...
		$dbh->do("INSERT INTO icmphdr (sid,cid,icmp_type,icmp_code) VALUES ('$sensorid','$cid','$d_read[13]','$d_read[14]'')") or croak($dbh->errstr);
		# tag hash population
		$proto="ICMP";
	}
	...
	$dbh->do("INSERT INTO data (sid,cid,data_payload) VALUES ('$sensorid','$cid','$pkthex')") or croak($dbh->errstr);
	$dbh->do("UPDATE sensor SET last_cid = '$cid' WHERE sid = '$sensorid'") or croak($dbh->errstr);
